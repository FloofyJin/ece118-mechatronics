...
#define TRACK_WIRE_1_OUT AD_PORTV3 
#define TRACK_WIRE_2_OUT AD_PORTV4
#define MAX_OUTPUT_VOLTAGE 1023
...
uint8_t TemplateCheckBattery(void) {
    ...
    uint16_t batVoltage = AD_ReadADPin(TRACK_WIRE_1_OUT); // read the battery voltage
...
uint8_t TrackWire(void) {
    ...
    if (trackWireVal < MAX_OUTPUT_VOLTAGE) { // if the output is less than the max voltage
    ...
    if (curEvent != lastEvent) { // check for change from last time
        thisEvent.EventType = curEvent;
        thisEvent.EventParam = trackWireVal;
        returnVal = TRUE;
        lastEvent = curEvent; // update history
#ifndef EVENTCHECKER_TEST           // keep this as is for test harness
        PostTrackWireService(thisEvent);
