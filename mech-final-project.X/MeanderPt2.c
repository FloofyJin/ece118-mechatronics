...
uint8_t InitMeanderPt2SubHSM(void) {
    ES_Event returnEvent;

    CurrentState = InitMeanderState;
    returnEvent = RunMeanderPt2SubHSM(INIT_EVENT);
...
ES_Event RunMeanderPt2SubHSM(ES_Event ThisEvent) {
    ...
    switch (CurrentState) {
        ...
        case SweepBackwards:
            switch (ThisEvent.EventType) {
                case (ES_ENTRY):
                    ES_Timer_InitTimer(HSM_TIMER, 1200);
                    Drive(-90, -90);
                    break;
                case (ES_TIMEOUT):
    ...
    if (makeTransition == TRUE) { // making a state transition, send EXIT and ENTRY
        ...
        RunMeanderPt2SubHSM(EXIT_EVENT); // <- rename to your own Run function
        CurrentState = nextState;
        RunMeanderPt2SubHSM(ENTRY_EVENT); // <- rename to your own Run function
    }
